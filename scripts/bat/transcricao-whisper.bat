...
for %%F in ("%input_dir%\*.mp4") do (
    ...
    for %%i in ("%%~nF") do (
        ...
        :: Move o arquivo MP4 para a pasta criada
        set "arquivo_mp4=%%~nxF"
        move "%%F" "!output_dir!\!arquivo_mp4!"
        
        :: Executa o Whisper para o arquivo atual
        echo Processando: !arquivo_mp4!
        whisper "!output_dir!\!arquivo_mp4!" ^
            --model medium ^
            --device cuda ^
            --language pt ^
            --output_dir "!output_dir!"
        
        echo Transcrição concluída para !arquivo_mp4! Os arquivos foram salvos em: !output_dir!
        echo.
    )
)
